...
class DatabaseConnection {
  ...
  async healthCheck() {
    ...
    if (this.client && this.client.topology) {
      try {
        const server = this.client.topology.s.description;
        poolStats = {
          connections: {
            total: this.client.topology.s.pool ? this.client.topology.s.pool.totalConnectionCount : 0,
            available: this.client.topology.s.pool ? this.client.topology.s.pool.availableConnectionCount : 0,
            pending: this.client.topology.s.pool ? this.client.topology.s.pool.pendingConnectionCount : 0,
          },
          serverInfo: {
            type: server.type,
            state: server.state,
        ...
        console.warn('Could not retrieve pool statistics:', error.message);
      }
    }
    ...
    return {
      ...
      poolStats,
      config: {
        maxPoolSize: connectionConfig.mongodb.options.maxPoolSize,
        minPoolSize: connectionConfig.mongodb.options.minPoolSize,
        maxIdleTimeMS: connectionConfig.mongodb.options.maxIdleTimeMS,
      }
    };
  }
}
